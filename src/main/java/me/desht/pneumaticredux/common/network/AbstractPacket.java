...
public abstract class AbstractPacket<REQ extends AbstractPacket> implements IMessage, IMessageHandler<REQ, REQ> {
    ...
    public REQ onMessage(final REQ message, final MessageContext ctx) {
        if (ctx.side == Side.SERVER) {
            PneumaticRedux.proxy.addScheduledTask(() -> message.handleServerSide(message, ctx.getServerHandler().player), true);
        } else {
            PneumaticRedux.proxy.addScheduledTask(() -> message.handleClientSide(message, PneumaticRedux.proxy.getPlayer()), false);
        }
        return null;
    }
