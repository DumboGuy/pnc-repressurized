...
import me.desht.pneumaticredux.common.network.FilteredSynced;
...
public class TileEntityPressureChamberInterface extends TileEntityPressureChamberWall implements IGUITextFieldSensitive, IRedstoneControlled {
    ...
    private PressureChamberInterfaceHandler inventory = new PressureChamberInterfaceHandler();
    @DescSynced
    @LazySynced
    ...
    private void exportToInventory() {
        ...
        if (te != null) {
            ...
            ItemStack leftoverStack = PneumaticCraftUtils.exportStackToInventory(te, inventory.getStackInSlot(0), side);
    ...
    private void importFromChamber(TileEntityPressureChamberValve core) {
        ...
        for (ItemStack chamberStack : chamberStacks) {
            ...
            if (inputStack.isEmpty() || (inputStack.isItemEqual(chamberStack)) && inventory.test(0, chamberStack)) {
     ...
     * wasNotNull = teValve != null; super.setCore(core); if(getWorld().isRemote)
    ...
    public List<String> getProblemStat() {
        ...
        if (interfaceMode == EnumInterfaceMode.NONE) {
            textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a77The Interface can't work.", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
            textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a70-The Interface is not in a properly formed Pressure Chamber, and/or", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
            textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a70-The Interface is not adjacent to an air block of the Pressure Chamber, and/or", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
            textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a70-The Interface isn't orientated right.", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
        } else if (!redstoneAllows()) {
            textList.add("gui.tab.problems.redstoneDisallows");
        } else if (!enoughAir) {
            textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a77There's not enough pressure in the Pressure Chamber to move the items.", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
            textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a70Apply more pressure to the Pressure Chamber. The required pressure is dependent on the amount of items being transported.", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
        }
        return textList;
    }
    ...
    private class PressureStackHandler extends FilteredItemStackHandler {
        public PressureStackHandler() {
    ...
    public void handleGUIButtonPress(int guiID, EntityPlayer player) {
        if (guiID == 1) {
            ...
            inventory.test(0, new ItemStack(Items.STICK));
    ...
    private class PressureChamberInterfaceHandler extends FilteredItemStackHandler {
        ...
        public boolean test(Integer slot, ItemStack itemStack) {
            switch (filterMode) {
                case ITEM:
                    boolean filterEmpty = true;
                    for (int i = 0; i < 9; i++) {
                        ItemStack filterStack = getStackInSlot(i + 1);
                        if (!filterStack.isEmpty()) {
                            filterEmpty = false;
                            if (itemStack.isItemEqual(filterStack)) {
                                return true;
