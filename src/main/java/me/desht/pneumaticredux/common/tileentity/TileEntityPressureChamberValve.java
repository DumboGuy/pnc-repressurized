...
import me.desht.pneumaticredux.api.recipe.IPressureChamberRecipe;
...
import me.desht.pneumaticredux.common.block.BlockPressureChamberValve;
import me.desht.pneumaticredux.common.block.Blockss;
import me.desht.pneumaticredux.common.block.IBlockPressureChamber;
        ...
        /*for(int i = 0; i < multiBlockSize - 2; i++) {
            for(int j = 0; j < multiBlockSize - 2; j++) {
                for(int k = 0; k < multiBlockSize - 2; k++) {
                    Block blockID = getWorld().getBlock(i + multiBlockX + 1, j + multiBlockY + 1, k + multiBlockZ + 1);
                    if(!blockID.isAir(getWorld(), i + multiBlockX + 1, j + multiBlockY + 1, k + multiBlockZ + 1)) {
                        ItemStack stack = new ItemStack(blockID, 1, getWorld().getBlockMetadata(i + multiBlockX + 1, j + multiBlockY + 1, k + multiBlockZ + 1));
