...
import me.desht.pneumaticredux.api.client.pneumaticHelmet.InventoryTrackEvent;
import me.desht.pneumaticredux.api.drone.AmadronRetrievalEvent;
import me.desht.pneumaticredux.api.drone.DroneConstructingEvent;
...
import me.desht.pneumaticredux.common.item.ItemAmadronTablet;
...
import me.desht.pneumaticredux.common.network.PacketPlaySound;
import me.desht.pneumaticredux.common.network.PacketSetMobTarget;
...
import me.desht.pneumaticredux.common.remote.GlobalVariableManager;
import me.desht.pneumaticredux.common.thirdparty.ModInteractionUtilImplementation;
...
//        }
    ...
    public void quetziMoo(ServerChatEvent event) {
        if (event.getUsername().equals("Quetzz") && event.getMessage().equals("m00")) {
            for (int i = 0; i < 4; i++)
                NetworkHandler.sendTo(new PacketPlaySound(SoundEvents.ENTITY_COW_AMBIENT, event.getPlayer().posX, event.getPlayer().posY, event.getPlayer().posZ, 1, 1, true), event.getPlayer());
        }
    }
