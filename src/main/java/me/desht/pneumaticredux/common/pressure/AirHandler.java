...
import me.desht.pneumaticredux.api.tileentity.IAirListener;
...
import me.desht.pneumaticredux.common.network.PacketSpawnParticle;
...
public class AirHandler implements IAirHandler {
    ...
    private int getUpgrades(EnumUpgrade upgrade) {
        return TileEntityBase.getUpgrades(parentInventory, upgrade);
    }
    ...
    public void validate(TileEntity parent) {
        parentInventory = parent.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, EnumFacing.DOWN) ?
                parent.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, EnumFacing.DOWN) : null;
//        parentInventory = parent instanceof IInventory ? (IInventory) parent : null;
    ...
    public void airLeak(EnumFacing side) {
        ...
        if (soundCounter <= 0) {
            soundCounter = 20;
            NetworkHandler.sendToAllAround(new PacketPlaySound(Sounds.LEAKING_GAS_SOUND, getPos().getX(), getPos().getY(), getPos().getZ(), 0.1F, 1.0F, true), getWorld());
        }
