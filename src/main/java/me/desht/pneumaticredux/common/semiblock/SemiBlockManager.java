...
import me.desht.pneumaticredux.common.network.PacketSetSemiBlock;
...
public class SemiBlockManager {
    ...
    static SemiBlockManager getServerInstance() {
        return INSTANCE;
    }
    ...
    static Item registerSemiBlock(String key, Class<? extends ISemiBlock> semiBlock, boolean addItem) {
        ...
        if (addItem) {
            ...
            PneumaticRedux.proxy.registerSemiBlockRenderer(item);
    ...
    static void registerItems(IForgeRegistry<Item> registry) {
        for (Item item : semiBlockToItems.values()) {
    ...
    static Item getItemForSemiBlock(ISemiBlock semiBlock) {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        if (this == getServerInstance()) getClientOldInstance().onClientTick(event);
            ...
            EntityPlayer player = PneumaticRedux.proxy.getPlayer();
            if (player != null) {
                for (ISemiBlock semiBlock : addingBlocks) {
                    Chunk chunk = semiBlock.getWorld().getChunkFromBlockCoords(semiBlock.getPos());
                    getOrCreateMap(chunk).put(semiBlock.getPos(), semiBlock);
