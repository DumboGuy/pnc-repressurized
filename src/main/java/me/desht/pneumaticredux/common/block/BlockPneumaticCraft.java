...
import me.desht.pneumaticredux.common.tileentity.IComparatorSupport;
...
public abstract class BlockPneumaticCraft extends Block implements IPneumaticWrenchable, IUpgradeAcceptor, IPeripheralProvider {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (player.isSneaking() || getGuiID() == null || isRotatable() && (player.getHeldItem(hand).getItem() == Itemss.MANOMETER || ModInteractionUtils.getInstance().isModdedWrench(player.getHeldItemMainhand().getItem())))
            ...
            if (!world.isRemote) {
                ...
                if (te != null && hand == EnumHand.MAIN_HAND && !FluidUtils.tryInsertingLiquid(te, player, hand, returnedItems)) {
                    player.openGui(PneumaticRedux.instance, getGuiID().ordinal(), world, pos.getX(), pos.getY(), pos.getZ());
                } else {
    ...
    protected int getInventoryDropStartSlot(IItemHandler inventory) {
        return 0;
    }

    protected int getInventoryDropEndSlot(IItemHandler inventory) {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List<String> curInfo, boolean extraInfo) {
        if (PneumaticRedux.proxy.isSneakingInGui()) {
