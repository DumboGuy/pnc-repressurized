...
import me.desht.pneumaticredux.common.inventory.ContainerChargingStation;
import me.desht.pneumaticredux.common.item.IChargingStationGUIHolderItem;
    ...
    protected void addProblems(List<String> textList) {
        ...
        if (chargeStack.isEmpty()) {
            ...
            if (chargeItem.getPressure(chargeStack) > te.getPressure() + 0.01F && chargeItem.getPressure(chargeStack) <= 0) {
                textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a77The put in item can't be discharged", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
                ...
                textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a77The put in item can't be charged", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
                textList.add("\u00a70The item is full.");
            } else if (!te.charging && !te.disCharging) {
                textList.addAll(PneumaticCraftUtils.convertStringIntoList("\u00a77The put in item can't be (dis)charged", GuiConstants.MAX_CHAR_PER_LINE_LEFT));
