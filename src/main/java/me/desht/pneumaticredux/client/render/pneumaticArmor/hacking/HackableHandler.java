...
import me.desht.pneumaticredux.client.render.pneumaticArmor.hacking.block.*;
import me.desht.pneumaticredux.client.render.pneumaticArmor.hacking.entity.*;
...
public class HackableHandler {
    ...
    private static HackableHandler getInstance() {
        if (PneumaticRedux.proxy.getSide() == Side.CLIENT) {
    ...
    private static boolean isInDisplayCooldown(IHackableEntity hackableBlock, Entity entity) {
        if (entity.world.isRemote) {
            ...
            int requiredHackTime = hackableBlock.getHackTime(entity, PneumaticRedux.proxy.getPlayer());
            return target != null && target.getHackTime() >= requiredHackTime && target.getHackTime() <= requiredHackTime + 20;
        } else {
            return false;
        }
